#' gfpca_TwoStep
#' 
#' Implements a two-step approach to generalized functional principal components analysis for 
...
#' @param basis Basis used as FPCs; in simulations, allows the use of the true FPCs
  ...
  if (type=="naive")
    {
      ...
      W.gamm.fpca <- matrix(rep(FIT.MU.gamm.fpca, I), nrow=I, byrow=T) + Z.gamm.fpca
  ...
  else if (type=="approx")
    {
      ...
      W.gamm.hmy <- matrix(rep(FIT.MU.gamm.hmy, I), nrow=I, byrow=T) + Z.gamm.hmy
  ...
  else
    {
      ...
      W.gamm.fixed <- matrix(rep(FIT.MU.gamm.fixed, I), nrow=I, byrow=T) + Z.gamm.fixed
